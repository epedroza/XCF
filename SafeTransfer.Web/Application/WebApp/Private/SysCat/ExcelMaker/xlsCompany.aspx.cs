...
namespace SafeTransfer.Web.Application.WebApp.Private.SysCat.ExcelMaker
{
   public partial class xlsCompany : System.Web.UI.Page
   {
      ...
      // Utilerías
		...
		Boolean HasValidParams(String sKey, ref ENTCompany oENTCompany){
            ...
            oENTCompany.sNombre  = sKey.Split(new Char[] { '|' })[0].ToString();
            oENTCompany.tiActivo = Int16.Parse(sKey.Split(new Char[] { '|' })[1].ToString());
		...
		void ExportExcel(DataTable oDataTable, String sSheetName){
            ...
            sGeneralTitle = "SafeTransfer - Compañías al " + DateTime.Now.ToString("d");
		...
		void SelectCompany(ENTCompany oENTCompany){
			...
			try{
				...
				// Transacción exitosa
				ExportExcel(oENTResponse.dsResponse.Tables[2], "Company");
		...
		protected void Page_Load(object sender, EventArgs e){
			...
			try
			{
				...
				if (!HasValidParams(this.Request.QueryString["key"].ToString(), ref oENTCompany)){
					this.Response.Redirect("~/Application/WebApp/Private/SysApp/saNotificacion.aspx?key=" + sDefaultErrorMessage, false);
				}
				
				// Consultar Órdenes de Compra
				SelectCompany(oENTCompany);
